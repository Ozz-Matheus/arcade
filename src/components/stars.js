...
export class Stars {
	...
	update() {
		...
		Object.keys(this.stars).forEach(starType => {
			this.stars[starType].getChildren().forEach(star => {
		            if (star.y > this.relatedScene.scale.parentSize.height * 1.1) {
		            star.setY(Stars.APPEAR_ABOVE);
		            star.setX(Phaser.Math.Between(0, this.relatedScene.scale.parentSize.width));
		        }
		    });
		});
	...
	initialize(star) {
	    ...
	    star.setX(Phaser.Math.Between(0, this.relatedScene.scale.parentSize.width));
	    star.setY(Phaser.Math.Between(0, this.relatedScene.scale.parentSize.height));
	}
